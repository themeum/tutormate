...
const Installation = ({ percentage, pluginInfo }) => {
	...
	const renderLoader = () => {
		return Object.keys(pluginInfo).map((pluginName) => {
			...
			let { pluginState, responseStatus, title } = plugin;
			if (pluginState === 'active') {
				...
				if (responseStatus === 'initial') {
					...
					return (
						<div className="plugin-item" key={pluginName}>
							<span class="dashicons dashicons-no-alt plugin-not-installed">Not Installing</span>
							<div className="title">{title ? title : __('Loading...', 'tutormate')}</div>
						</div>
					);
				}
			...
			<div className="modal-content">
				{percentage === 100 ? (
					...
					<div className="modal-head">
						<h4>
							<span>
								{__('Hold on a moment', 'tutormate')} {<SVGLoader />}
							</span>
							{__('Your site is installing...', 'tutormate')}
						</h4>
					</div>
				)}
